  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      function(position) {
        ...
        getJSON("https://api.openaq.org/v1/latest?coordinates=" + lat + "," + long + "&radius=10000",
          function(err, data) {
            if (err) {
              document.getElementById('modal-body-content').innerHTML = "Unable to access API";
              return;
            } else {

              if (data.results) {
                if (data.meta.found < 1) {
                  document.getElementById('modal-body-content').innerHTML = "No air quality data found near you";
                  return;
                } else {
                  var measurements = data.results[0].measurements;
                  var subIndices = [];
                  for (var i = 0; i < measurements.length; i++) {
                    subIndices.push(getSubIndex(measurements[i].parameter, measurements[i].value));
                  }
                  ...
                  if (subIndices.length % 2 == 0) {
                    ...
                    data2 = subIndices.slice(subIndices.length / 2, subIndices.length);
                  } else {
                    data1 = subIndices.slice(0, subIndices.length / 2);
                    data2 = subIndices.slice(subIndices.length / 2 + 1, subIndices.length);
                  }
                  ...
                  document.getElementById('aqi-display').innerHTML = "Current AQI is " + index.toFixed(3);

                  document.getElementById('category-display').style.color = 'white';
                  if (index < 50) {
                    document.getElementById('category-display').innerHTML = "AQI category is Good";
                    document.getElementById('category-display').style.background = 'green';
                  } else if (index < 100) {
                    document.getElementById('category-display').innerHTML = "AQI category is Moderate";
                    document.getElementById('category-display').style.background = 'yellow';
                  } else if (index < 150) {
                    document.getElementById('category-display').innerHTML = "AQI category is Unhealthy For Sensitive Groups";
                    document.getElementById('category-display').style.background = 'orange';
                  } else if (index < 200) {
                    document.getElementById('category-display').innerHTML = "AQI category is Unhealthy";
                    document.getElementById('category-display').style.background = 'red';
                  } else if (index < 300) {
                    document.getElementById('category-display').innerHTML = "AQI category is Very Unhealthy";
                    document.getElementById('category-display').style.background = 'purple';
                  } else {
                    document.getElementById('category-display').innerHTML = "AQI category is Hazardous";
                    document.getElementById('category-display').style.background = 'maroon';
                  }
                  ...
                  document.getElementById('extra-info').innerHTML += "Your air quality data was found at " + location + "<br>";
